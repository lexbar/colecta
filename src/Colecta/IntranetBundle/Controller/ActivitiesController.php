        ...
        if(count($users))
        {
            ...
            foreach($users as $u)
            {
                if($u->getRole() && $u->getRole()->getName() != 'ROLE_BANNED')
                {
                    ...
                    $pointsRequest = $em->createQuery('SELECT p FROM ColectaUserBundle:Points p WHERE p.user = :user AND p.date >= \''.$year.'-01-01 00:00:00\' AND p.date <= \''.$year.'-12-31 23:59:59\'')->setParameter('user', $u)->getResult();
